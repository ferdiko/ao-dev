...
This workspace contains the two UI surfaces for ao:
...
- Node.js 20+
- npm
- for the standalone web app, a running ao backend from the repo root

## Install

From `src/user_interfaces/`:
...
The standalone app lives in `src/user_interfaces/web_app/`.
...
1. From the repo root, start or restart the backend:
   ...
   ```
2. From `src/user_interfaces/`, start the frontend:
   ```bash
   npm run dev:webapp
   ```
...
Build the extension from `src/user_interfaces/`:
...
The extension-specific guide lives at [`docs/user-guide/vscode-extension.md`](/Users/jub/ao-dev/docs/user-guide/vscode-extension.md).
