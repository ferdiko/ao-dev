...
A project is identified by a `.sovara/.project_id` file in the project root
...
from sovara.common.constants import PROJECT_CONFIG_DIR, PROJECT_ID_FILE


def find_project_root(start_path: str) -> str | None:
    """Walk up from start_path looking for .sovara/.project_id. Return the directory or None."""
...
def read_project_id(project_root: str) -> str:
    """Read project UUID from .sovara/.project_id."""
...
def delete_project_configs(project_locations: list[str]) -> None:
    """Delete .sovara/ directories at the given project locations."""
...
def write_project_id(project_root: str, project_id: str) -> None:
    """Write project UUID to .sovara/.project_id."""
...
def ensure_project_configured(user_id: str, script_dir: str) -> dict:
    ...
    Flow:
    1. Walk up from script_dir looking for .sovara/.project_id
    ...
    if project_root is None:
        ...
        if result:
            ...
            logger.info(f"Restored .sovara/.project_id at {project_location}")

    if project_root is None:
        # No project found anywhere — prompt to create
        print("\nNo Sovara project found. Let's create one.\n")
        ...
        if row:
            ...
            print(f"\nSovara project found at {project_root} but not yet configured.\n")
