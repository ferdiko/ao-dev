...
def _playbook_request(method: str, endpoint: str, data: dict = None, sse: bool = False) -> dict:
    """Make HTTP request to ao-playbook server.

    Args:
        sse: If True, send Accept: text/event-stream and parse SSE response.
             Used for create/update/delete which may stream validation progress.
    """
...
def handle_add_lesson(server, msg: dict, conn: socket.socket) -> None:
    ...
    result = _playbook_request("POST", endpoint, data, sse=True)
...
def handle_update_lesson(server, msg: dict, conn: socket.socket) -> None:
    ...
    result = _playbook_request("PUT", endpoint, data, sse=True)
...
def handle_delete_lesson(server, msg: dict, conn: socket.socket) -> None:
    ...
    result = _playbook_request("DELETE", f"/lessons/{lesson_id}", sse=True)
    if "error" in result:
        send_json(conn, {"type": "lesson_error", "error": result.get("error", "Unknown error")})
    else:
...
def handle_folder_ls(server, msg: dict, conn: socket.socket) -> None:
    """List folder contents via ao-playbook POST /lessons/folders/ls."""
    path = msg.get("path", "")
    result = _playbook_request("POST", "/lessons/folders/ls", {"path": path})
